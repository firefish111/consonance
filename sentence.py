...
def sentence_to_tones(secs_per_syllable, volume, text, *, stop_pause=0, break_pause=0, interword_pause=0, f_filter=(lambda x: x)):
  filtered = text \
    .replace(".", " !FULLSTOP ") \
    .replace(",", " !COMMA ") \
    .replace(";", " !COMMA ") \
  ...
  for word in filtered.split():
    ...
    for fm in (phones_list.vowels[phone] for phone in w.vowels()):
      ...
      if len(fm) >= 2:
        tones.append(synth.synth(secs_per_syllable, volume, *f_filter(fm[0]), interpolate_with=f_filter(fm[1])))
      else: # otherwise we pretend nothing's happened
        tones.append(synth.synth(secs_per_syllable, volume, *f_filter(fm[0])))
